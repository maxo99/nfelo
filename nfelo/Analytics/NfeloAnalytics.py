...
class NfeloAnalytics:
    ...
    def flatten(self):
        ...
        flat["all_time_game_number"] = (
            flat.groupby(["team"])["season_game_number"].cumcount() + 1
        )
        ## return ##
        return flat
    ...
    def add_rolls(self, df):
        ...
        for metric in ["margin", "wepa_margin", "pff_point_margin"]:
            ## ytd ##
            df["{0}_ytd".format(metric)] = df.groupby(["team", "season"])[
                metric
            ].transform(lambda x: x.cumsum())
            ## rolling ##
            for roll in [8, 16]:
                df["{0}_L{1}".format(metric, roll)] = df.groupby(["team", "season"])[
                    metric
                ].transform(lambda x: x.rolling(roll, min_periods=1).sum())
        ## return ##
        return df
    ...
    def add_nfelo(self, df):
        ...
        df = pd.merge(
            df,
            nfelo_df[["team", "game_id", "starting_nfelo", "ending_nfelo"]].rename(
                columns={"starting_nfelo": "nfelo_start", "ending_nfelo": "nfelo"}
            ),
            on=["team", "game_id"],
            how="left",
        )
    ...
    def save_team_file(self):
        ...
        ## full ##
        self.team_file.to_csv("{0}/Data/Formatted Data/team_file.csv".format(loc))
        ## most recent ##
