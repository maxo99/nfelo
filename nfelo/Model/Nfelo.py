...
class Nfelo:
    ...
    def project_game(self, row):
                ...
                self.reversion_records.append(
                    {
                        ...
                        "league_elo": statistics.median(
                            self.yearly_elos[row["season"] - 1]
                        ),
                        "mean_reverted_elo": (
                        ...
                        "dvoa_elo": 1505
                        + 484 * row["{0}_projected_dvoa".format(team_type)],
        ...
        initial_elo_dif = (
            ...
            +
            ## qb adj ##
            self.config["qb_weight"] * (row["home_538_qb_adj"] - row["away_538_qb_adj"])
        )
        ...
        row["nfelo_home_probability_base"] = elo_to_prob(
            elo_dif=initial_elo_dif, z=self.config["z"]
        )
        ...
        mr_regression_open = regress_to_market(
            ...
            row["home_line_open"],
            ## regression config ##
            self.config["market_regression"],
            self.config["min_mr"],
            self.config["spread_delta_base"],
            self.config["rmse_base"],
            self.config["long_line_inflator"],
            self.config["hook_certainty"],
            ## errors ##
        ...
        row["nfelo_home_line_open"] = probability_to_spread(
            row["nfelo_home_probability_open"]
        )
        row["nfelo_home_probability_close"] = elo_to_prob(row["nfelo_dif_close"])
        row["nfelo_home_line_close"] = probability_to_spread(
            row["nfelo_home_probability_close"]
        )
        ## calc evs ##
        ## Open ##
        (
            row["home_loss_prob_open"],
            row["home_push_prob_open"],
            row["home_cover_prob_open"],
        ) = calc_cover_probs(row["nfelo_home_line_open"], row["home_line_open"])
        row["home_open_ev"] = (
            row["home_cover_prob_open"] - 1.1 * row["home_loss_prob_open"]
        ) / 1.1
        row["away_open_ev"] = (
            row["home_loss_prob_open"] - 1.1 * row["home_cover_prob_open"]
        ) / 1.1
        ## close ##
        (
            row["home_loss_prob_close"],
            row["home_push_prob_close"],
            row["home_cover_prob_close"],
        ) = calc_cover_probs(row["nfelo_home_line_close"], row["home_line_close"])
        ...
        row["home_close_ev"] = (
            row["home_cover_prob_close"] - 1.1 * row["home_loss_prob_close"]
        ) / 1.1
        row["away_close_ev"] = (
            row["home_loss_prob_close"] - 1.1 * row["home_cover_prob_close"]
        ) / 1.1
        ## calc clvs ##
    ...
    def process_game(self, row):
        ...
        if pd.isnull(row["home_margin"]) or pd.isnull(row["away_margin"]):
            raise Exception(
                "NFELO PROCESS ERROR: Attempted to process an unplayed game"
            )
        ## calculate shifts ##
        ## home ##
        weighted_shift_home = calc_weighted_shift(
            ...
            self.config["k"],
            self.config["b"],
            self.config["market_resist_factor"],
            True,
        )
        ## away ##
        ...
        ## reset rolling info ##
        self.current_elos[ht]["starting_nfelo_adj"] = self.current_elos[ht][
            "ending_nfelo_adj"
        ]
        self.current_elos[ht]["starting_model_se"] = self.current_elos[ht][
            "ending_model_se"
        ]
        self.current_elos[ht]["starting_market_se"] = self.current_elos[ht][
            "ending_market_se"
        ]
        ## update ##
        ...
        ## reset rolling info ##
        self.current_elos[at]["starting_nfelo_adj"] = self.current_elos[at][
            "ending_nfelo_adj"
        ]
        self.current_elos[at]["starting_model_se"] = self.current_elos[at][
            "ending_model_se"
        ]
        self.current_elos[at]["starting_market_se"] = self.current_elos[at][
            "ending_market_se"
        ]
        ## update ##
    ...
    def project_spreads(self):
        ...
        ## apply nfelo ##
        print(
            "Projecting Week {0}, {1}".format(
                unplayed.iloc[0]["week"], unplayed.iloc[0]["season"]
            )
        )
