...
from ...Utilities import elo_to_prob, probability_to_spread, calc_shift
...
def market_resist_explore():
    ...
    for home_elo_dif_model in range(-25, 26):
        ...
        for home_elo_dif_market in range(-25, 26):
            ...
            try:
                ...
                print(
                    "Could not calc for model_dif: {0} // market_dif: {1}".format(
                        home_elo_dif_model, home_elo_dif_market
                    )
                )
    ## save ##
    ...
    df.to_csv("{0}/market_resist.csv".format(pathlib.Path(__file__).parent.resolve()))
