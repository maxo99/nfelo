...
class DataLoader:
    ...
    def __init__(self):
        ...
        self.intermediate_data_loc = "{0}/Intermediate Data".format(
        ...
        self.db = dcm.load(
            [
                ...
                "qbelo",  ## nfelo models
                "filmmargins",
                "market_data",  ## other data
            ]
        )
        self.dvoa_projections = pd.read_csv(
            "{0}/dvoa_projections.csv".format(self.intermediate_data_loc), index_col=0
        )
    ...
    def format_market_data(self):
        ...
        if len(market_data[pd.isnull(market_data["home_line_close"])]) > 0:
            print(
                "     Warning -- {0} games were missing a spread".format(
                    len(market_data[pd.isnull(market_data["home_line_close"])])
                )
            )
            print(
                "                /n".join(
                    market_data[pd.isnull(market_data["home_line_close"])][
                        "game_id"
                    ].tolist()
                )
            )
        ...
        (
            ...
            market_data["home_ml_open"], market_data["away_ml_open"]
        )
        (
            ...
            market_data["home_ml_close"], market_data["away_ml_close"]
        )
        ...
        market_data["spread_implied_away_win_probability_open"] = (
            1 - market_data["spread_implied_home_win_probability_open"]
        )
        market_data["spread_implied_home_win_probability_close"] = (
            spread_to_probability(market_data["home_line_close"])
        )
        market_data["spread_implied_away_win_probability_close"] = (
            1 - market_data["spread_implied_home_win_probability_close"]
        )
        ## combine implied
        market_data["home_implied_win_probability_open"] = market_data[
            "spread_implied_home_win_probability_open"
        ].combine_first(market_data["ml_implied_home_win_probability_open"])
        market_data["away_implied_win_probability_open"] = (
            1 - market_data["home_implied_win_probability_open"]
        )
        market_data["home_implied_win_probability_close"] = market_data[
            "spread_implied_home_win_probability_close"
        ].combine_first(market_data["ml_implied_home_win_probability_close"])
        market_data["away_implied_win_probability_close"] = (
            1 - market_data["home_implied_win_probability_close"]
        )
        ## add implied elo dif ##
        market_data["home_implied_elo_dif_open"] = prob_to_elo(
            market_data["home_implied_win_probability_open"]
        )
        market_data["home_implied_elo_dif_close"] = prob_to_elo(
            market_data["home_implied_win_probability_close"]
        )
        market_data.to_csv("{0}/market_data.csv".format(self.intermediate_data_loc))
        ## return ##
        return market_data
    ...
    def format_games(self):
        ...
        games = (
            ...
            .rename(columns={"gameday": "game_date", "weekday": "game_day"})
            .copy()
        )
        ...
        games["is_playoffs"] = numpy.where(games["type"] == "post", 1, 0)
        ## return ##
        return games
        ...
        games = pd.merge(
            ...
            self.market_data[
                [
                    ...
                    "home_line_open",
                    "home_line_close",
                    "home_line_close_price",
                    "away_line_close_price",
                    "home_ml_close",
                    "away_ml_close",
                    "home_ats_pct",
                    "home_implied_win_probability_open",
                    "home_implied_win_probability_close",
                    "home_implied_elo_dif_open",
    ...
    def add_dvoa(self, games):
        ...
        games = pd.merge(
            games,
            self.dvoa_projections[["team", "season", "projected_total_dvoa"]].rename(
                columns={
                    "team": "home_team",
    ...
    def add_pff_margins(self, games):
        ...
        games = pd.merge(
            games,
            self.db["filmmargins"][["game_id", "team", "film_margin"]].rename(
                columns={
                    "team": "home_team",
    ...
    def add_hfa(self, games):
        ...
        games["is_neutral"] = numpy.where(games["location"] == "Neutral", 1, 0)
        ## translate to elo ##
    ...
    def add_qbs(self, games):
        ...
        if len(missing) > 0:
            print(
                "          Warning - {0} games were missing qb adjs".format(
                    len(missing)
                )
            )
            print("                    Filling with 0")
    ...
    def add_game_numbers(self, games):
        ...
        flat = (
            pd.concat(
                [
                    self.db["games"][["home_team", "season", "week", "game_id"]]
                    .copy()
                    .rename(columns={"home_team": "team"}),
                    self.db["games"][["away_team", "season", "week", "game_id"]]
                    .copy()
                    .rename(columns={"away_team": "team"}),
                ]
            )
            .sort_values(by=["team", "season", "week"], ascending=[True, True, True])
    ...
    def gen_current_file(self):
        ...
        ## save ##
        games.to_csv("{0}/current_file.csv".format(self.intermediate_data_loc))
        ## return ##
        return games
