...
class NfeloFormatter:
    ...
    def gen_rolling_hfa(self):
        ...
        """
        self.data.current_file[["season", "week", "hfa_base"]].groupby(
            ["season", "week"]
        ).agg(rolling_hfa=("hfa_base", "median")).reset_index().to_csv(
    ...
    def gen_scored_games(self):
        ...
        self.graded.graded_games[
            [
                ...
                "qbelo_se",
                "nfelo_close_se",
                "nfelo_unregressed_se",
                "market_se",
                "market_open_se",
                "538_su",
                "qbelo_su",
                "nfelo_close_su",
                "nfelo_unregressed_su",
                "market_su",
                "market_open_su",
                "market_home_line",
                "qbelo_home_line",
                "nfelo_close_home_line",
        ...
        ).to_csv("{0}/scored_individual_games.csv".format(self.output_loc))
    ...
    def gen_wepa_flat(self):
        ...
        flat = flat.sort_values(
            by=["team", "season", "game_number"], ascending=[True, True, True]
        ).reset_index(drop=True)
        ## save ##
        flat.to_csv("{0}/wepa_flat_file.csv".format(self.output_loc))
    ...
    def gen_wt_ratings(self):
        ...
        ].to_csv("{0}/wt_ratings.csv".format(self.output_loc))
    ...
    def gen_projections(self):
        ...
        proj = (
            self.model.projections[
                [
                    ...
                    "home_line_open",
                    "home_line_close",
                    "nfelo_home_line_close",
                    "nfelo_home_line_base",
                    "nfelo_spread_delta",
                    "nfelo_home_probability_close",
                    "starting_nfelo_home",
                    "starting_nfelo_away",
                    "nfelo_dif_pre_adjustment",
                    "nfelo_dif_base",
                    "market_regression_factor_close",
                    "nfelo_dif_close",
                    ...
                    "home_538_qb",
                    "away_538_qb",
                    "away_loss_prob_close",
                    "away_push_prob_close",
                    "away_cover_prob_close",
                    "away_close_ev",
                    "home_loss_prob_close",
                    "home_push_prob_close",
                    "home_cover_prob_close",
                    "home_close_ev",
                    "old_game_id",
                    "game_date",
                    "game_day",
        ...
        ## add bet size ##
        proj["bet_size"] = bet_size(proj["home_cover_prob"], proj["home_loss_prob"])
        ## save ##
        proj.to_csv(
        ...
        pt["projected_winner"] = numpy.where(
            pt["nfelo_projected_home_win_probability"] >= 0.5,
            pt["home_team"],
            pt["away_team"],
        )
        pt["projected_winner_probability"] = numpy.round(
            numpy.where(
        ...
        pt.to_csv(
            "{0}/prediction_tracker.csv".format(self.external_folder), index=False
        )
        ## add to historics ##
        hist = pd.read_csv(
        ...
        hist = hist[
            ~numpy.isin(hist["game_id"], proj["game_id"].unique().tolist())
        ].copy()
        ## append ##
        ...
        flat = pd.concat(
            [
                proj[
                    ["season", "week", "home_team", "home_nfelo_elo", "home_538_qb_adj"]
                ].rename(
                    columns={
                        "home_team": "team",
                ...
                proj[
                    ["season", "week", "away_team", "away_nfelo_elo", "away_538_qb_adj"]
                ].rename(
                    columns={
                        "away_team": "team",
        ...
        try:
            existing = pd.read_csv(
                "{0}/elo_snapshot.csv".format(self.external_folder), index_col=0
            )
            ## merge ##
            existing = existing[
                ~numpy.isin(existing["team"], flat["team"].unique().tolist())
            ].copy()
            if len(existing) == 0:
                flat.sort_values(by=["nfelo"], ascending=[False]).reset_index(
                    drop=True
                ).to_csv("{0}/elo_snapshot.csv".format(self.external_folder))
            else:
                pd.concat([flat, existing]).sort_values(
                    by=["nfelo"], ascending=[False]
    ...
    def gen_most_recent_elo_file(self):
        ...
        flat["nfelo_soy"] = flat.groupby(["team", "season"])[
            "nfelo_starting"
        ].transform("first")
        ## add deltas ##
        ...
        ## save ##
        flat.to_csv("{0}/most_recent_elo_file.csv".format(self.output_loc))
    ...
    def gen_cur_w_analytics(self):
        ...
        df = (
            self.model.updated_file[
                [
                    ...
                    "home_net_wepa",
                    "away_net_wepa",
                    "home_net_wepa_point_margin",
                    "away_net_wepa_point_margin",
                    ...
                    "nfelo_home_line_base",
                    "qbelo_prob1",
                    "nfelo_home_probability_base",
                    "538_home_line_close",
                    "qbelo_home_line_close",
                ]
            ]
            .copy()
            .rename(
        ...
        ## save ##
        df.to_csv("{0}/current_file_with_analytics.csv".format(self.output_loc))
    ...
    def gen_nfelo_games(self):
        ...
        market = (
            self.data.market_data[
                [
                    ...
                    ## spreads ##
                    "home_line_open",
                    "home_line_open_price",
                    "away_line_open_price",
                    "home_line_close",
        ...
        ## merge ##
        df = pd.merge(base, market, on=["game_id"], how="left")
        ## add hfa base ##
        hfa = (
            self.data.db["hfa"][["game_id", "hfa_base"]]
            .rename(columns={"hfa_base": "hfa_base_mod"})
            .copy()
        )
        hfa["hfa_base_mod"] = 25 * hfa["hfa_base_mod"]  ## gross up to elo value
        df = pd.merge(df, hfa, on=["game_id"], how="left")
        ...
        ## save ##
        df.to_csv("{0}/nfelo_games.csv".format(self.external_folder))
