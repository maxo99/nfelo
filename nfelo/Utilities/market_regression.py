...
def initial_mr_factor(
    ...
    * mr_factor (float): the percent regress the model the market by
    ...
    ## calc factor ##
    return 4 / (1 + (spread_delta_base * spread_dif**2)) + spread_dif / 14
...
def rmse_adj(
    ...
    model_se_home: float,
    market_se_home: float,
    model_se_away: float,
    market_se_away: float,
) -> float:
    """
...
def regress_to_market(
    ...
    * mr_factor_used (float): The amount of regression used
    ...
    regression_factor_used = max(min_regression, min(1, market_regression * mr_factor))
    ## regress ##
