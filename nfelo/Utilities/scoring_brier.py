...
def brier_score_vector(probability: pd.Series, margin: pd.Series) -> pd.Series:
    """
...
def brier_score_number(probability: (float), margin: (float)) -> float:
    """
...
def brier_score(probability: (float or pd.Series), margin: (float or pd.Series)):
    """
...
def adj_brier(brier: float, corr: float):
    """
    ...
    * adj_brier: brier, rewarded for decreasing correlation
    """
    return brier * (1 + (1 - corr**2))


def ats_adj_brier(brier: float, ats_be: float, ats_play_pct: float):
    """
    ...
    * ats_adj_brier: brier, rewarded for better ats
    """
    return brier * (1 + 5 * (ats_play_pct * (ats_be - 0.53)))
