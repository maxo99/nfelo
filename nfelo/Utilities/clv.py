...
def calc_clv(original_home_spread: float, current_home_spread: float):
    ...
    * clv_away: the CLV for the away team
    ...
    home_loss_prob_open, home_push_prob_open, home_cover_prob_open = calc_cover_probs(
        proj_spread=original_home_spread, market_spread=original_home_spread
    )
    ## at close ##
    ...
    clv_home = (
        ## current nominal EV ##
        (home_cover_prob * 1 + home_push_prob * 0 + home_loss_prob * -1.1)
        -
    ...
    clv_away = (
        (away_cover_prob * 1 + away_push_prob * 0 + away_loss_prob * -1.1)
        - (
