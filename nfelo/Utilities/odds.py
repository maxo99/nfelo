...
def american_to_prob_vector(series: pd.Series):
    ...
    """
    return numpy.where(
        series < 100, (-1 * series) / (100 - series), 100 / (100 + series)
    )
...
def american_to_hold_adj_prob(
    home_odds: (pd.Series or int), away_odds: (pd.Series or int)
):
    """
    ...
    * hold(series, or float): Hold
    ...
    return (home_prob / combo_prob, away_prob / combo_prob, combo_prob - 1)
...
def american_to_price_vector(odds: pd.Series):
    ...
    """
    return numpy.where(odds < 0, numpy.absolute(odds), 100 / (odds / 100))
